...
namespace ServiceStack
{
    ...
    public static class Bundler
    {
        ...
        public static Func<bool> IsServiceStack = IsServiceStackAtWebRoot;
        ...
        private static bool? isServiceStackWebApp;
        public static bool IsServiceStackAtWebRoot()
        {
            if (isServiceStackWebApp == null)
            {
                Type endpointHostType = Type.GetType("ServiceStack.WebHost.Endpoints.EndpointHost");
                if (endpointHostType == null)
                {
                    return (isServiceStackWebApp = false).Value;                    
                }

                var pi = endpointHostType.GetProperty("Config", BindingFlags.Static);
                var config = pi.GetGetMethod().Invoke(null, new object[]{});
                pi = config.GetType().GetProperty("ServiceStackHandlerFactoryPath", BindingFlags.Public | BindingFlags.Instance);
                var ssPath = pi.GetGetMethod().Invoke(config, new object[]{});
                isServiceStackWebApp = ssPath == null;
            }
            return isServiceStackWebApp.Value;
        }
        ...
        public delegate object ObjectCtorDelegate(object args);
        private static ObjectCtorDelegate RouteValueDictionaryFactoryFn;
        
        public static IDictionary<string, object> ToRouteValueDictionary(this object attrs)
        {
            if (RouteValueDictionaryFactoryFn == null)
            {
                var type = Type.GetType(IsServiceStack()
                    ? "ServiceStack.Html.RouteValueDictionary"
                    : "System.Web.Routing.RouteValueDictionary");

                RouteValueDictionaryFactoryFn = Expression.Lambda<ObjectCtorDelegate>(
                    Expression.New(type), Expression.Parameter(typeof(object))
                  ).Compile();
            }
